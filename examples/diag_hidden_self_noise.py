"""Hidden self-noise diagnostic (2026-09-19), per user's detailed design spec: compares PEDM
OR/per-step scores under a checkpoint's LEARNED Concealment policy vs a forced-ZERO Concealment
(o_rep = obs_aug exactly, no forgery at all), with the SAME learned Disruptor policy and same
seeds in both conditions. If OR_learned >> OR_zero, the learned hidden policy is itself actively
creating detectable evidence (a genuine convergence problem for Concealment). If OR_zero is ALSO
high, the issue is more likely detector calibration / threshold / estimator drift than the
hidden policy specifically.

Usage:
    python -m examples.diag_hidden_self_noise --run_dir results/safety_marl/SafetyPointGoal1Gymnasium-v0/mappo_lagr/harl_native_v17_scmarl_loose --episodes 15
"""
import argparse
import glob
import json
...
def latest_seed_dir(run_dir):
    if os.path.isdir(os.path.join(run_dir, "models")):
        return run_dir
    return sorted(glob.glob(os.path.join(run_dir, "seed-*")))[-1]
...
def main():
    ...
    config = json.load(open(os.path.join(seed_dir, "config.json"), encoding="utf-8"))
    ...
    def run(seed, zero_hidden, max_steps=1000):
        ...
        for t in range(max_steps):
            if zero_hidden:
                phi_hid = np.zeros(full_dim, np.float32)
            else:
            ...
            o_rep = obs_aug + phi_hid * scale_full
            a_v = victim.act_from_augmented(obs_aug)  # role-split: victim always uses TRUE obs_aug
    ...
    for label, zero_hidden in [("learned Concealment", False), ("zero Concealment", True)]:
        rets, costs, per_step, ors = [], [], [], []
        for i in range(args.episodes):
            obs_seq, act_seq, ret, tc = run(9000 + i, zero_hidden)
            scores = pedm.predict_scores(obs_seq, act_seq)
            per_step.append(float(np.mean(scores)))
            ors.append(float(np.max(scores)))
            rets.append(ret)
            costs.append(tc)
        n_flag = sum(1 for s in ors if s > OR_THR)
        print(f"\n=== {label} ===")
        print(f"mean_ret={np.mean(rets):.2f} mean_true_cost={np.mean(costs):.2f} "
              f"per-step={np.mean(per_step):.4f} OR={np.mean(ors):.4f} flagged={n_flag}/{args.episodes}")
