#!/bin/bash
# 5-seed SC-MARL alternating (main method) runs for HalfCheetah-v4 and Ant-v4 (2026-09-21).
...
#     convention, which silently assumed ~1000-step episodes).
...
#     Ant clean floor ~0.0043 -> eps_cost=0.04. Do NOT reuse HalfCheetah's eps_cost for Ant (or
#     any other env) without re-measuring that env's own clean floor first (see
#     `examples/train_pedm_detector.py`'s sibling truedyn-floor measurement pattern in this
#     project's own memory/session history, not scripted as a standalone tool yet).
...
#
# Usage:
#   bash scripts/run_hc_ant_5seed.sh [n_rollout_threads] [max_concurrent_jobs]
...
for seed in 1 2 3 4 5; do
    queue_job "ant_scmarl_alt_s${seed}" python -u -m examples.train --algo mappo_alt --env mujoco_marl \
        --exp_name harl_native_ant_scmarl_alt_5seed_s${seed} \
...
wait
echo "=================== HC + Ant, 5 seeds each COMPLETE $(date) ==================="
echo "ALL DONE"
